...
/** Minimal theme surface frameBox needs (fg/bg only — no bold). */
...
/**
 * Bordered overlay box — inlined rather than imported from a shared package
 * (a file-based, non-npm extension can't resolve packages that only live in
 * the shared ~/.pi/agent/npm/node_modules tree). Same shape as
 * status-line.ts's frameBox. No scrolling: the overlay body is always
 * exactly TOOL_ORDER.length rows, which always fits.
 */
function frameBox(lines: string[], width: number, theme: FrameTheme): string[] {
	...
	// Rows carry several individually-colored segments (cursor, glyph, name,
	// value, bar); theme.fg/bold may emit a full reset (\x1b[0m) or bg-reset
	// (\x1b[49m) between them, so re-open the box's background after each one
	// or it punches transparent holes in the row.
	const SENTINEL = "\x00";
...
export function buildOptHelp(handles: Record<OptimizerTool, OptimizerHandle>): string {
	...
	return ["optimizer — token tools", "", ...lines].join("\n");
}
...
export function registerOptCommand(
	...
	pi.registerCommand("optimizer", {
		description: "optimizer: caveman / rtk / toon / ponytail tools",
				...
				(
					...
					return {
						render: () => {
							...
							return frameBox(lines, boxW, theme);
						},
						invalidate: () => {},
